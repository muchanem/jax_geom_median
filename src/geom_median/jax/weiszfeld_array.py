...
def geometric_median_array(points, weights, eps=1e-6, maxiter=100, ftol=1e-20):
    ...
    for _ in range(maxiter):
        ...
        norms = jnp.stack([jnp.linalg.norm((p - median).reshape(-1)) for p in points])
        new_weights = weights / jax.lax.clamp(eps, norms, jnp.inf)
...
def weighted_average(points, weights):
    ...
    """
    return jnp.average(points, weights=weights, axis=0)


def geometric_median_objective(median, points, weights):
    return jnp.average(jnp.concatenate([jnp.linalg.norm((p - median).reshape(-1)) for p in points]), weights=weights)
